...
- The mapping presentation in the UI displays exactly the same 12 columns, in the same order and spelling, as the processed output CSV.
- `Tariff Price` and `Premium Price` in the Mapping table display the final normalized EUR/MWh/h values from the published output, not source-currency values or processing evidence.
...
- P.36.21 (strict EUR normalization of Tariff Price and Premium Price) is implemented and
  ...
  Result opens it. The separate managed PRISMA download 5000-row large-export defect (P.36.14) was not touched;
  the active manual CSV import path has no internal 5000-row cap.

## Current blockers and risks — note on P.38/P.39/P.37

P.38 (2026-08-16) removed the entire managed PRISMA browser/download workflow (see its own section above). Every outstanding real-Windows/real-PRISMA validation item recorded above against P.36.2, P.36.3's managed-download support, P.36.8's managed-download trigger path, P.36.13, P.36.14 (including the 5,000-row large-export defect and its P.36.22 fix), and P.36.20 is now moot: that code no longer exists and cannot be validated or shipped. Their historical implementation and validation records remain below and in CHANGELOG.md as evidence of what was built and tested at the time, per the Maintenance note's preservation rule — they are not instructions for further work. P.36.4 (manual CSV selection), P.36.8's mapping-display UI itself, P.36.15, and P.36.16 are unaffected in scope by P.38/P.39 and their own outstanding real-Windows validation items (real-PRISMA CSV) remain live and current. P.36.19's Mapping-display Currency/Rate columns are superseded by the 2026-08-16 P.37 correction: Mapping now displays the exact authoritative 12 output columns only, while rate-resolution modules remain available only as historical processing evidence paths. P.36.21's strict-EUR-gate *contract* (fail closed, never guess, never publish source-currency as EUR) is unaffected, but its Tariff/Premium *resolution mechanism* is superseded by P.37 (2026-08-16, same day) — see P.37's own section above for the current mechanism and its own outstanding real-ECB validation item. P.39 (same day) further removed the "Import PRISMA Export"/"Open Result" buttons and the export-date picker; see its own section below. Any earlier text below still describing the two-step Select-CSV-then-Import-PRISMA-Export flow, the export-date control, or Open Result reflects pre-P.39 history, not current behavior.
...
- Fails closed exactly as P.36.21 already required: a row whose currency/date pair has no confirmed ECB rate blocks the entire batch, with no partial output; a missing/unsupported currency or unparseable auction date can never reach `price_normalization.py` in the first place, since `processor.py` already rejects such a row at import time.

**Reused unchanged:** `ecb_rates.py` in full — its quotation-direction inversion (ECB's published "currency per EUR" figure inverted into "EUR per currency unit") and its weekend/ECB-holiday fallback (`endPeriod`-bounded query to the latest prior publication) required no changes at all, since both were already generically keyed by `(currency, date)`. `price_normalization.py`'s `format_price`/`PRICE_DECIMAL_PLACES`/`ROUND_HALF_UP` serialization, `PriceNormalizationOutcome`/`NormalizedPrice`/`PriceConversionFailure`/`PriceNormalizationResult` dataclasses, Decimal-arithmetic/input-validation conventions, and the overall fail-closed BLOCKED-carries-nothing batch contract are all unchanged in spirit, adapted only to the new per-side resolution/conversion path. `prisma_output.transform_row`/`OUTPUT_CSV_COLUMNS` and `prisma_publication.py`'s merge/dedup/atomic-publish contract are untouched; the 12-column output contract itself is unchanged.

**Superseded:** `rate_resolution.resolve_rates_for_rows`/`resolve_auction_rate` and `prisma_auction_lookup.PrismaAuctionLookup` are no longer in the Tariff/Premium EUR call graph (`price_normalization.normalize_prices_for_output` no longer accepts `reference_catalog`/`auction_lookup`/`page`, and neither does `prisma_output.write_prisma_output`/`prisma_publication.publish_cumulative_output`/`prisma_import_workflow.run_prisma_import_workflow`, all of which forwarded them only for that purpose). The 2026-08-16 P.37 Mapping correction also removes the separate Mapping-display `Currency`/`Rate to EUR`/`Rate Date` columns: the visible Mapping table now reloads from the cumulative published output CSV after successful processing and displays all accepted, non-duplicate rows using exactly the authoritative 12 output fields, including final normalized EUR/MWh/h `Tariff Price` and `Premium Price`. The active manual CSV import path reads every selected CSV record until end-of-file, with no 5000-row cap in CSV import, validation, booked-capacity filtering, P.37 ECB normalization, persistence, cumulative publication, Mapping reload, or Mapping display. The old same-`source_date` whole-file exclusivity rule is withdrawn: multiple distinct valid PRISMA CSV files for the same source date or selected period may be imported sequentially, exact re-imports are processed safely and idempotently, and partially overlapping files add only new distinct rows. Deduplication is row-record based at persistence/publication boundaries, never by filename, file hash, source date, or whole-file identity. The workflow summary now reports complete-file counts separately for source rows, accepted rows, filtered rows, rejected rows, publication deduplications, and newly inserted rows.

**Changed files:** `processor.py` (per-side currency-tagged price parsing, new `_PRICE_UNITS` table), `storage.py` (new `EcbAuctionDateRateRecord`/`ecb_auction_date_rates` table/methods; `_translate_legacy_price_fields` extended to drop the new P.37-only row keys before the dormant `auctions`-table upsert; source-operation storage now permits multiple `(source_date, sha256)` rows and migrates the old date-unique table), `price_normalization.py` (new `(auction_date, currency)` resolution/conversion path, replacing the Auction-ID-keyed one for this module only), `prisma_output.py`/`prisma_publication.py`/`prisma_import_workflow.py` (dropped the now-unused `auction_lookup`/`page` parameters; `prisma_publication.py` additionally dropped `reference_catalog`, which it only ever forwarded to normalization; the 2026-08-16 no-cap correction exposes publication deduplication counts and surfaces complete-file source/accepted/filtered/rejected/deduplicated/newly-inserted counts in the workflow summary; the same-date cumulative correction removes source-date exclusivity from active imports), `prisma_source_updates.py` (source lifecycle validation no longer rejects accepted same-date or older distinct files before row processing), `mapping_presentation.py`/`ui_components.py`/`app.py` (Mapping reloads and displays the cumulative 12-column published output without row caps).

**Outstanding:** real-Windows/real-PRISMA/real-ECB validation through the real UI (same posture as the existing outstanding P.36.19/P.36.21 items).
