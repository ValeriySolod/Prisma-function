...
| P.34.2 | Maximize the managed browser window | 🟡 In progress | Chromium launches with `--start-maximized`, and Playwright uses the native window size without a fixed viewport. Regression coverage verifies both launch settings. | Run focused and full automated checks, then confirm Chrome and Edge use the full available Windows workspace. |
