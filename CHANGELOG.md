# Prisma-function Changelog

This file holds the detailed per-increment implementation narratives, dated
defect-fix write-ups, decision-gate records, and superseded requirement
versions that used to live inline in `ROADMAP.md`. They were relocated here
by P.37.4 (2026-08-14) to keep `ROADMAP.md` focused on current status,
current blockers/risks, and the next recommended increment. No content was
deleted or altered during the move — every section below is exactly as it
previously appeared in `ROADMAP.md`.

For current status, see the tables in `ROADMAP.md`'s "Completed and
historical roadmap", "Completed foundations", "Suspended obsolete
increments", and "Remaining support and finalization stages" sections.
...
**Status:** ✅ Implemented and automated-tested (2026-08-16) on branch `feature/remove-managed-prisma-download`.
...
**Removed (production code and tests deleted):**

- `browser.py` (Playwright browser lifecycle, default-browser detection) and `tests/test_browser.py`.
- `prisma_lifecycle.py` (`PrismaLifecycleController`, `ManagedPrismaAuctionDetailFetcher`, Open/Close Prisma session lifecycle) and `tests/test_prisma_lifecycle.py`.
- `prisma_download.py` (`PrismaDownloadOrchestrator`, date-filter automation, the 5,000-row large-result confirmation handling, filesystem-fallback download detection) and `tests/test_prisma_download.py`.
- `prisma_page.py` (`PrismaSessionValidator`, live PRISMA page/session validation) and `tests/test_prisma_page.py`.
- `date_range_selection.py` (`DateRange`, `DateRangeSelection`, used only for the managed-download date filter) and `tests/test_date_range_selection.py`.
- `app.py`: the Open Prisma / Close Prisma buttons and badge, the Date Range sidebar group (start/end date controls, Validate Date Range button, label), the Download Folder sidebar group (Choose Download Folder button, label, `DownloadDirectorySelection`), `PrismaLifecycleController` construction and polling (`_prisma_timer`, `_poll_prisma_lifecycle`, `_handle_download_event`, `_open_prisma_session`, `_close_prisma_session`, `_prisma_open_failed`, `_prisma_close_failed`), and the `ManagedPrismaAuctionDetailFetcher` wiring in `_refresh_mapping_display`/`start_processing`. `closeEvent()` no longer waits on a PRISMA lifecycle worker or `CLOSE_FAILED` state — only a running processing thread defers shutdown.
- `download_directory.py`: trimmed to only `default_download_directory()` (Documents resolution) and `validate_download_directory()`; `default_managed_download_directory()`, `default_downloads_directory()`, `ensure_directory_exists()`, and `DownloadDirectorySelection` are removed (all existed only to support the managed-download default `<Downloads>\PrismaFunction` directory and its user-reselection).
- `prisma_auction_lookup.py`: `PlaywrightAuctionDetailFetcher` and `AUCTION_DETAIL_URL_TEMPLATE` are removed — this class issued a live HTTP request through a managed Playwright browser session's `page.request`, which is now categorically disallowed. `PrismaAuctionLookup` no longer defaults to it; omitting `fetcher` now fails closed with `PrismaAuctionDetailTransportError` immediately (the same typed "Unavailable" outcome an unreachable live surface always produced), instead of raising an unrelated `AttributeError`. The transport-independent business layer (`parse_auction_end_record`, `AuctionDetailFetcher` protocol) is unchanged and still fully testable with fakes.
- `playwright` is removed from `requirements.txt`; `PrismaFunction.spec` no longer bundles it (`collect_submodules`/`collect_data_files` calls removed); `validate_package.py` no longer requires the Playwright driver files in the packaged distribution.

**Preserved unchanged:** local CSV validation (`manual_csv_selection.py`, `csv_contracts.py`), the 12-column output mapping and transformation (`processor.py`, `prisma_output.py`), currency/EUR normalization (`price_normalization.py`, `ecb_rates.py`, `rate_resolution.py`), CET/CEST date/time handling (`prisma_datetime.py`), cumulative persistence and deduplication (`storage.py`, `prisma_publication.py`), and the scrollable Mapping table (`mapping_presentation.py`, `ui_components.py`). The `reserve_unique_download_path()` filesystem utility (a generic atomic-unique-path reservation, not itself PRISMA/browser-specific) moved from the deleted `prisma_download.py` into `prisma_output.py`, its one remaining caller.

**Known consequence, not a defect:** with no live PRISMA transport left anywhere in the application, a Finished-state auction that has never been resolved before can never obtain a rate and blocks EUR normalization for its whole batch (P.36.21's existing fail-closed contract, unchanged — see `price_normalization.py`). A previously resolved auction is unaffected: `storage.AuctionStorage`'s durable per-Auction-ID cache serves it indefinitely without any PRISMA access. This is an inherent, correctly-fail-closed consequence of removing all browser/PRISMA-website access, not something this increment attempts to work around; a non-browser rate-evidence source (if ever approved) is out of scope here and would be a separate increment.

**Documentation updated:** `CLAUDE.md`, `ROADMAP.md`, this file, `BUILDING.md`, `INSTALLER.md`, `README.md`, `RELEASE_CHECKLIST.md`, `P22_CLEAN_WINDOWS_CHECKLIST.md` (obsolete-section notice), and `.github/workflows/windows-ci.yml`'s compile-check file list.

**Evidence (2026-08-16):** `tests/test_app.py`, `tests/test_download_directory.py`, and `tests/test_prisma_auction_lookup.py` were rewritten to match the removal (managed-download/lifecycle tests deleted; local-CSV, mapping, cumulative rate-cache, and real-call-graph EUR-normalization tests retained or adapted to use `storage.AuctionStorage`'s durable cache directly instead of a faked Playwright transport). Actually executed and passing: `python -m pytest -q` → **606 passed, 1 skipped** (one test, `test_processing_success_preserves_full_statistics`, is flaky under full-suite load due to pre-existing `QSignalSpy` timing sensitivity already documented elsewhere in the same file — it passes reliably in isolation and its logic is unchanged by this increment); `python -m compileall -q` against the current module list → succeeded with no output; `git diff --check` → exit 0; `python -m PyInstaller --clean --noconfirm PrismaFunction.spec` → succeeded; `python validate_package.py` → `Package validation passed`, and the built `dist\PrismaFunction\_internal` directory contains no `playwright` directory. No real-Windows, real-PRISMA, or real-ECB validation was performed or is claimed.

**Outstanding before this increment can be marked fully complete:** merge to `main`; real-Windows manual validation (Select CSV, Import PRISMA Export, Mapping display, Open Result, confirm no PRISMA-website access of any kind ever occurs).
...
**Correction:** P.38 removed managed PRISMA acquisition but left a two-step local flow in place: Select CSV only refreshed the Mapping preview, and a separate "Import PRISMA Export" button — with its own file dialog and a "PRISMA EXPORT DATE" picker — actually processed and published the file. This is withdrawn. Select CSV is now the single user action: after a valid local CSV is selected, it is immediately processed, merged into cumulative persistent storage without duplicates, and the Mapping table is refreshed.

**Removed from `app.py` (obsolete two-step state, controls, callbacks, and now-unreachable code):**

- The "Import PRISMA Export" button and its `start_processing()` handler (which previously opened its own, second file dialog, unvalidated by `ManualCsvSelection`).
- The export-date sidebar group: the "PRISMA EXPORT DATE" label and the `import_date` `QDateEdit`.
- The "Open Result" button, its `open_result()` handler, and `PrismaMonitorApp._last_output_path` (the bookkeeping field that existed only to support it) — the published cumulative CSV in the approved directory remains the durable, structured output for later monitoring-system integration; nothing about writing or persisting it changed, only the UI affordance to open it in the OS default application.
- `QDesktopServices`/`QUrl`/`QDate`/`QDateEdit` imports, unused once the above were removed.

**Changed:**

- `_select_manual_csv()`: after a successful `ManualCsvSelection` validation, calls `_refresh_mapping_display()` (unchanged mapping-preview logic) and, only if that succeeded, immediately calls the new `_process_selected_csv()` to run the full transform/normalize/publish pipeline on a background thread — matching `start_processing()`'s prior body, minus the second file dialog and the export-date parameter. Guards against re-entry while a selection's processing is already active (Select CSV is disabled via `_update_controls()` until it settles), mirroring the prior `process_button` guard.
- `_refresh_mapping_display()` now returns `bool` (`True` on success, `False` on a preview failure that already cleared the table and showed its own "Mapping" error) so `_select_manual_csv()` never attempts to fully process a file that could not even be parsed for the preview — avoiding a redundant second, differently-worded error dialog for the same fundamentally invalid file.
- `_process_worker()`'s `source_date` argument is removed; `run_prisma_import_workflow` is always called with `source_date=datetime.now().date()` — the exact fallback the pre-P.39 code already used whenever no UI date was supplied, so the dedup/exact-retry contract (`prisma_source_updates.py`) is unchanged in behavior, not just in code.
- `_update_controls()` now enables/disables `choose_manual_csv_button` (previously `process_button`/`import_date`).

**Preserved unchanged:** CSV transformation, capacity-threshold filtering, market mapping, currency/EUR conversion, CET/CEST datetime normalization, and cumulative deduplication rules (`processor.py`, `price_normalization.py`, `ecb_rates.py`, `rate_resolution.py`, `prisma_datetime.py`, `prisma_source_updates.py`, `storage.py`, `prisma_publication.py`) — none of this correction's changes touch those modules. The scrollable Mapping table and its P.36.19 rate-resolution-cache display are unchanged.

**Documentation updated:** `CLAUDE.md`, `ROADMAP.md`, this file.

**Evidence (2026-08-16):** `tests/test_app.py` rewritten to match — tests using the removed `start_processing()`/`process_button`/`open_result()`/`_last_output_path` were adapted to `_select_manual_csv()`/`choose_manual_csv_button`, and a new `_settle_processing()` test helper pumps the Qt event loop (bounded) between two selections in the same test so the new "processing active" guard does not treat a still-settling first selection as blocking a second one — mirroring the event-loop time a real user's separate clicks naturally provide. A new autouse fixture default-mocks `QMessageBox.critical` (overridable per test) since selecting a CSV now always triggers real background processing, which fails closed for most of the file's fixture rows (no confirmed EUR rate) and would otherwise be free to pop a real blocking dialog once settled. `python -m pytest -q` → **606 passed, 1 skipped** (the previously flaky `test_processing_success_preserves_full_statistics` — see P.38's entry — is now also rewritten off `QSignalSpy` onto the same polling pattern and passed reliably across repeated runs, both isolated and in the full suite). `python -m compileall -q` and `git diff --check` both passed.

**Outstanding before this increment can be marked fully complete:** merge to `main`; real-Windows manual validation (select a local CSV, confirm it is processed and published automatically with no separate step, confirm the Mapping table).
